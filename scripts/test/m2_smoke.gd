...
func _run() -> void:
	...
	check(World.visibility_at(Vector2i(34, 10), player.global_position) == 0.0, "fog of war: interior out of sight range = invisible")
	check(World.visibility_at(Vector2i(-3, 5), player.global_position) == 15.0, "exterior/background fully revealed at any distance")
	check(World.line_of_sight(player.global_position, Vector2i(20, 5)), "clear line of sight along the player's own floor")
	check(not World.line_of_sight(player.global_position, Vector2i(16, 8)), "the floor slab blocks line of sight downward")
	check(World.visibility_at(Vector2i(16, 8), player.global_position) == 0.0, "room below the floor stays hidden even in sight range")
	# Obstacles (wood/plastic) only attenuate sight; structure blacks out.
	player.inventory.add("wood_block", 2)
	World.place_block("wood_block", Vector2i(14, 5))
	World.place_block("wood_block", Vector2i(14, 4))
	var behind := World.visibility_at(Vector2i(12, 4), player.global_position)
	check(behind > 3.0 and behind < 12.0, "sight bleeds dimly past a wooden obstacle (%.1f)" % behind)
	World.remove_block(Vector2i(14, 5))
	World.remove_block(Vector2i(14, 4))
