...
Development has completed **M0–M2** and most of **M3** (generation + world runtime; still open:
persistence/save-load, minimap + map reveal, relay/central station shells, surface debris,
invisible edge walls, the two-jump validator, and the in-editor room-curation flow). The main
scene is now `scenes/city/city.tscn` — a seeded 2400×400 drowned city (`CityGen`, ~1.5 s,
deterministic; `--seed=N`, `--shot=path[:zoom]`); room templates live in `data/rooms.json` —
...
`tools/gen_rooms.py`; the generator selects by zone and filters by each room's depth range. The water sim is
