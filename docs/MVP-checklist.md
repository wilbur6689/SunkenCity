...
- [x] T0 Rooftops (2026-09-06): `band_at` returns `roof` for open air above the waterline outside tower footprints (pockets and the test tower stay `dry`); night roof spawns (`World._tick_roof_night`, knobs in `enemies.json seeding.roof_night_*`) hunt players outdoors on roofs unless they stand in a sealed room of their own; placeholder Prowler (`pounds: false`, roof stats only); covered in `district_smoke`
