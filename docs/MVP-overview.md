...
- Modifiers (2026-09-05, `docs/modifiers/`): family = the tower's district, tier = the floor's
  band (6 × 5 base + 30 hybrids); rarity = highest tier as title-text color (gray/green/blue/
  purple/gold); Modification Bench: sacrifice-to-stock, COMBINE (ladder or hybrid), APPLY to
  unmodified gear only. District weapons: 55 melee (craftable blank) + 38 ranged (firearms loot-only).
...
- Exact numbers for per-band tables and loot tables (the modifier grid and its stats are
  generated by `tools/gen_modifiers_json.py`; tune the tables there).
